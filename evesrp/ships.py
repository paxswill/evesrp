...
class ShipList(object):
    ...
    def __getitem__(self, key):
        if not isinstance(key, int):
            raise TypeError("Invalid typeID for ship lookup: '{}'".format(key))
        if key not in ShipList._ships:
            resp = current_app.requests_session.get(
                    'https://public-crest.eveonline.com/types/{}/'.format(key),
                    headers={'Accept':
                        'application/vnd.ccp.eve.ItemType-v3+json'})
            if check_crest_response(resp) and resp.status_code == 200:
                ShipList._ships[key] = resp.json()['name']
            else:
                message = "Cannot find the typeID requested [{}]: {}".format(
                        resp.status_code, key)
                raise KeyError(message)
        return ShipList._ships[key]
