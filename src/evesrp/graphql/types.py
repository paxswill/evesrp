...
class User(graphene.ObjectType):
    ...
    groups = graphene.NonNull(graphene.List(lambda: Group))
...
class Permission(graphene.ObjectType):
    ...
    permission = PermissionType(required=True)
    ...
    def from_model(cls, model):
        ...
        # library Enums
        permission_type = getattr(PermissionType, model.permission.name)
        return cls(permission=permission_type,
                   division=Division(id=model.division_id),
                   entity=Entity(id=model.entity_id))
